"""
Ethnicity estimator using Namsor API.

Filters preprocessed user names and calls the Namsor diaspora API
to estimate ethnicity groups based on first and last names.

Usage:
    python ethnicity_estimator.py <input_csv> <api_key> [--output output.csv] [--batch-size 100]

Example:
    python ethnicity_estimator.py unique_user_names_preprocessed.csv your_api_key_here

The script will create output CSV with ethnicity predictions.
"""
...
# Namsor API endpoint
NAMSOR_API_URL = "https://v2.namsor.com/NamSorAPIv2/api2/json/diasporaBatch"
...
def prepare_api_batch(all_rows: List[Dict[str, str]], batch_size: int = 100) -> List[List[Dict[str, Any]]]:
    ...
    for idx, row in enumerate(all_rows):
        ...
        # Skip if no last name (required by diaspora API)
        if not last_name:
            print(f"Warning: Skipping row {idx} - no last name found")
            continue
...
def call_namsor_api(batch: List[Dict[str, Any]], api_key: str) -> Optional[Dict[str, Any]]:
    """
    Call the Namsor diaspora API with a batch of names.
    ...
    try:
        ...
        if response.status_code == 401:
            ...
            print(f"Error: Insufficient credits or permission denied (403)")
        else:
...
def process_all_batches(batches: List[List[Dict[str, Any]]], api_key: str) -> List[Dict[str, Any]]:
    ...
    Returns:
        List of all personal names with ethnicity predictions
    """
    all_results = []
...
def merge_results_with_original(all_rows: List[Dict[str, str]], 
    ...
    Rows that were sent to the API get populated ethnicity columns;
    rows that were not sent get blank ethnicity columns.
    ...
    Returns:
        Merged list with all rows, ethnicity data filled where available
    """
    # Create a mapping from original row index to API result
    result_map = {int(r.get("id", -1)): r for r in api_results}
    
    ethnicity_fields = [
        'ethnicity', 'ethnicity_alt', 'ethnicities_top', 'score',
        'probability_calibrated', 'probability_alt_calibrated', 'script', 'lifted'
    ]
    ...
    for idx, row in enumerate(all_rows):
        ...
        if idx in result_map:
            ...
            merged_row['ethnicity'] = api_result.get('ethnicity', '')
            merged_row['ethnicity_alt'] = api_result.get('ethnicityAlt', '')
            merged_row['ethnicities_top'] = json.dumps(api_result.get('ethnicitiesTop', []))
            ...
            merged_row['lifted'] = api_result.get('lifted', '')
        else:
            # Blank ethnicity columns for rows not sent to API
...
def write_results_to_csv(merged_data: List[Dict[str, Any]], output_path: str):
    ...
    # Define fieldnames (original columns + new ethnicity columns)
    ...
    # Remove ethnicity fields from original fields if they exist
    original_fields = [f for f in original_fields if f not in ethnicity_fields]
    fieldnames = original_fields + ethnicity_fields
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Estimate ethnicity groups from names using Namsor API'
    )
    ...
    parser.add_argument('--output', '-o', help='Output CSV file (default: input_ethnicity.csv)')
    ...
    if not api_key:
        ...
        print("Provide it as an argument: python ethnicity_estimator.py input.csv YOUR_API_KEY")
        print("Or set the environment variable: $env:NAMSOR_API_KEY = 'your_api_key'")
        sys.exit(1)
    
    args.api_key = api_key  # Update args with resolved key
    ...
    if args.output:
        ...
        output_csv = f"{input_stem}_ethnicity.csv"
